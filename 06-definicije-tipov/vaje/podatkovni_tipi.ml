...
let rec intbool_map intbool_map f_int f_bool ib_list = 
  match ib_list with
  | Nil -> Nil
...
let rec intbool_list f_int f_bool =
  ...
  | Nil -> Nil
  | Int (i, ibl) -> Int (f_int i, mapper ibl)
  | Int (b, ibl) -> Int (f_bool b, mapper ibl)
  in mapper *)
...
let rec intbool_separate ib_list = 
  let rec sep acc_int acc_bool = function
    | Nil -> (acc_int, acc_bool)
    | Int (i, ibl) -> sep (i :: acc_int) acc_bool ibl
    | Bool (b, ibl) -> sep acc_int (b :: acc_bool) ibl
  in
...
let rec count_magic ws = 
  ...
  in
  magic_counter {fire=0; frost=0; arcane=0}
